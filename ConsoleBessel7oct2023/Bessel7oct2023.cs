namespace ConsoleBessel7oct2023
{
    internal class Bessel7oct2023_jy
    {
...
2.761695824829316e-2, 3.240517192670181e-4 };
        ...
        public double j0(double x)
        {
            ...
            else
            {
                ...
                return Program.sqrt(twoopi / ax) * (Program.cos(xx) * nump / denp - z * Program.sin(xx) * numq / denq);
            }
        }
        ...
        double j1(double x)
        {
            ...
            else
            {
                ...
                double ans = Program.sqrt(twoopi / ax) * (Program.cos(xx) * nump / denp - z * Program.sin(xx) * numq / denq);
        ...
        double y0(double x)
        {
            if (x < 8.0)
            {
                double j0x = j0(x);
                rat(x, y0r, y0s, 8);
                return nump / denp + twoopi * j0x * Program.log(x);
            }
            else
            {
                ax = x;
                asp(y0pn, y0pd, y0qn, y0qd, 1.0);
                return Program.sqrt(twoopi / x) * (Program.sin(xx) * nump / denp + z * Program.cos(xx) * numq / denq);
            }
        }
        ...
        double y1(double x)
        {
            if (x < 8.0)
            {
                double j1x = j1(x);
                rat(x, y1r, y1s, 7);
                return x * nump / denp + twoopi * (j1x * Program.log(x) - 1.0 / x);
            }
            else
            {
                ax = x;
        ...
        double jn(int n, double x)
        {
            ...
            {
                tox = 2.0 / ax;
                m = 2 * ((n + (int)(Program.sqrt(ACC * n))) / 2);
                jsum = false;
                bjp = ans = sum = 0.0;
                bj = 1.0;
                for (j = m; j > 0; j--)
